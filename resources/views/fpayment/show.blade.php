...
@section('breadcrumb')
    <li class="breadcrumb-item active"><i class="fa fa-id-card-o"></i> عرض الدفعة النقدية | الحوالة النهائية <i
            class="fa fa-address-card" aria-hidden="true"></i></li>
    <li class="breadcrumb-item active"><a href="{{ route('fpayment.list') }}">الدفعات النقدية | الحوالات النهائية <i class="fa fa-table"
                aria-hidden="true"></i></a></li>
    <li class="breadcrumb-item active"><a href="{{route('welcome')}}">الرئيسية <i class="fa fa-tachometer-alt"></i></a></li>
@endsection

@section('card-title')
    معلومات الدفعة النقدية | الحوالة النهائية
@endsection
...
@section('table')
    <table class="table table-bordered table-striped table-hover table-sm"
        ...
        <tr class="d-flex">
            ...
            <td class="col-md-4">{{ $payment->contract_number }}</td>
            <th class="col-md-2">تاريخ العقد</th>
            <td class="col-md-4">{{ $payment->contract_date }}</td>
        </tr>
        <tr class="d-flex">
...
@section('card-footer')
    @if (Auth::user()->hasPermission('final_records-input'))
        <div class="col-xs-1">
            <form action="{{ route('fpayment.releaseForm', ['id' => $payment->id]) }}" class="form-inline">
                <button type="Submit" class="btn btn-outline-primary">تحرير</button>
            </form>
        </div>
        <div class="col-xs-1">
            <form action="{{ route('fpayment.requiseForm', ['id' => $payment->id]) }}" class="form-inline">
    ...
    @endif
    <div class="col-xs-1">
        <form action="{{ route('fpayment.edit', ['id' => $payment->id]) }}" class="form-inline">
