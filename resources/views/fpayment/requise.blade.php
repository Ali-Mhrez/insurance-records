...
@section('title')
مصادرة الدفعة النقدية | الحوالة
@endsection

@section('breadcrumb')
<li class="breadcrumb-item active">مصادرة الدفعة | الحوالة <i class="fa fa-edit"></i></li>
<li class="breadcrumb-item ">
    <a href="{{ route('fpayment.show', ['id' => $payment->id]) }}"><i class="fa fa-id-card-o"></i> عرض الدفعة | الحوالة <i class="fa fa-address-card" aria-hidden="true"></i></a>
</li>
<li class="breadcrumb-item "><a href="/fpayments">الدفعات | الحوالات النهائية <i class="fa fa-table" aria-hidden="true"></i></a>
</li>
...
@section('form')
<form action="{{ route('fpayment.requise', ['id' => $payment->id]) }}">
    @csrf()
    <div class="row">
        <div class="form-group col-md-4">
            ...
            <select class="form-control @error('book_issued_by') is-invalid @enderror" id="book_issued_by" name="book_issued_by">
                ...
                <option value="{{ $key }}" @if ($key==old('book_issued_by', $payment->book_issued_by)) selected="selected" @endif>{{ $value }}</option>
                @endforeach
            </select>
