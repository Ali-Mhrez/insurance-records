...
@section('breadcrumb')
<li class="breadcrumb-item active">مصادرة الكفالة <i class="fa fa-edit"></i></li>
...
<li class="breadcrumb-item "><a href="{{ route('guarantee.list') }}">الكفالات البدائية <i class="fa fa-table" aria-hidden="true"></i></a>
</li>
...
@section('form')
<form action="{{ route('guarantee.requise', ['id' => $guarantee->id]) }}">
    @csrf()
    <div class="row">
        <div class="form-group col-md-4">
            ...
            <select class="form-control @error('book_issued_by') is-invalid @enderror" id="book_issued_by" name="book_issued_by">
                ...
                <option value="{{ $key }}" @if ($key==old('book_issued_by', $guarantee->book_issued_by)) selected="selected" @endif>{{ $value }}</option>
                @endforeach
            </select>
