@extends("templates.CUoperrations")
    ...
    @section('breadcrumb')
        <li class="breadcrumb-item active">إضافة شيك جديد <i class="fa fa-plus"></i></li>
        <li class="breadcrumb-item"><a href="{{route('welcome')}}">الرئيسية <i class="fa fa-tachometer-alt"></i></a></li>
    @endsection

    @section('card-title')
    إضافة شيك بدائي جديد
    @endsection

    @section('form')
    <form action="{{route('check.store')}}">
        @csrf()
        <div class="row">
        ...
        <div class="row">
            <div class="form-group col-md-4">
                ...
                <input type="number" step="0.01" class="form-control @error('value') is-invalid @enderror" id="value" name="value" placeholder="" value="{{ old('value') }}">
                @error('value')
                <li class=" alert alert-danger">{{ $message }}</li>
                @enderror
            </div>
        ...
        <div class="row">
            <div class="form-group col-md-4">
                <label for="matter">موضوع العقد | المناقصة</label>
