...
@section('breadcrumb')
    <li class="breadcrumb-item active"><i class="fa fa-id-card-o"></i> عرض الكفالة البدائية <i class="fa fa-address-card"
            aria-hidden="true"></i></li>
    <li class="breadcrumb-item active"><a href="{{ route('guarantee.list') }}">الكفالات البدائية 
        <i class="fa fa-table" aria-hidden="true"></i></a></li>
    <li class="breadcrumb-item active"><a href="{{route('welcome')}}">الرئيسية 
        <i class="fa fa-tachometer-alt"></i></a></li>
@endsection

@section('card-title')
    معلومات الكفالة البدائية
@endsection
...
@section('table')
    ...
    <table class="table table-bordered table-striped table-hover table-sm"
        ...
        <tr class="d-flex">
            ...
            <th class="col-md-2">المكافئ بالليرةالسورية</th>
...
@section('card-footer')
    @if (Auth::user()->hasPermission('initial_records-input'))
        <div class="col-xs-1">
            <form action="{{ route('guarantee.extendForm', ['id' => $guarantee->id]) }}" class="form-inline">
                <button type="Submit" class="btn btn-outline-primary">تمديد</button>
            </form>
        </div>
        <div class="col-xs-1">
            <form action="{{ route('guarantee.releaseForm', ['id' => $guarantee->id]) }}" class="form-inline">
                <button type="Submit" class="btn btn-outline-primary">تحرير</button>
            </form>
        </div>
        <div class="col-xs-1">
            <form action="{{ route('guarantee.monetizeForm', ['id' => $guarantee->id]) }}" class="form-inline">
                <button type="Submit" class="btn btn-outline-primary">تسييل</button>
            </form>
        </div>
        <div class="col-xs-1">
            <form action="{{ route('guarantee.requiseForm', ['id' => $guarantee->id]) }}" class="form-inline">
    ...
    @endif
    <div class="col-xs-1">
        <form action="{{ route('guarantee.edit', ['id' => $guarantee->id]) }}" class="form-inline">
